  ...
  if (loading) {
    return (
      <div className={className}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', fontSize: '0.8rem', color: theme.textSecondary }}>
          Loading...
        </div>
      </div>
    );
    ...
    <div className={className}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', height: 'calc(100% - 2rem)', overflowY: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {recentMatches.length === 0 ? (
          <div style={{ fontSize: '0.7rem', color: theme.textSecondary, textAlign: 'center', marginTop: '1rem' }}>No recent matches</div>
        ) : (
          recentMatches.map((match) => (
            <div key={match.id} style={{
              ...
              <div style={{ color: theme.textPrimary, marginBottom: '0.25rem', fontSize: '0.7rem' }}>
                <span style={{ color: theme.primaryColor }}>{match.winner_name}</span>
                {' def. '}
              ...
              <div style={{ color: theme.textTertiary, fontSize: '0.65rem', marginTop: '0.25rem' }}>
                {match.score}
              </div>
            </div>
          ))
