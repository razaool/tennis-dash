...
// Load ML model coefficients
let mlModel = null;
try {
  const modelPath = path.join(__dirname, '..', 'logistic_surface_model.json');
  mlModel = JSON.parse(fs.readFileSync(modelPath, 'utf8'));
  console.log('✓ ML model loaded');
} catch (error) {
  console.warn('⚠ ML model not found - prediction endpoint will be disabled');
}
...
app.post('/api/match-prediction', async (req, res) => {
  try {
    ...
    if (!['Hard', 'Clay', 'Grass'].includes(surface)) {
      return res.status(400).json({ error: 'Invalid surface. Must be Hard, Clay, or Grass' });
    }

    // Fetch player data and compute features
    const features = await computeMatchFeatures(player1_id, player2_id, surface, tournament_level);
    
    if (!features) {
      return res.status(400).json({ error: 'Unable to compute features for these players' });
    }

    // Make prediction using logistic regression
    const prediction = predictMatch(features, mlModel);

    res.json({
      player1_win_probability: prediction.player1_win_prob,
      player2_win_probability: prediction.player2_win_prob,
      predicted_winner: prediction.predicted_winner,
      confidence: prediction.confidence,
      features: features,
      model_info: {
        type: 'Logistic Regression',
        accuracy: mlModel.metrics.accuracy,
        brier_score: mlModel.metrics.brier,
        training_period: `${mlModel.train_start} to ${mlModel.train_end}`
      }
    });
...
/**
 * Helper function to compute match features
 */
async function computeMatchFeatures(player1_id, player2_id, surface, tournament_level) {
  try {
    // Get player details
    const playerQuery = `
      SELECT id, name, birth_date, height
      FROM players
      WHERE id IN ($1, $2)
    `;
    const playerResult = await pool.query(playerQuery, [player1_id, player2_id]);
    ...
    const p1 = players[player1_id];
    const p2 = players[player2_id];

    // Calculate age difference
    const age1 = p1.birth_date ? (new Date().getFullYear() - new Date(p1.birth_date).getFullYear()) : 25;
    const age2 = p2.birth_date ? (new Date().getFullYear() - new Date(p2.birth_date).getFullYear()) : 25;
    const age_diff = age1 - age2;
    ...
    `;
    const eloResult = await pool.query(eloQuery, [player1_id, player2_id, surface]);
    
    const elos = {};
    eloResult.rows.forEach(r => {
      elos[r.player_id] = parseFloat(r.rating_value);
    });

    const p1_elo = elos[player1_id] || 1500;
    const p2_elo = elos[player2_id] || 1500;
    const surface_elo_diff = p1_elo - p2_elo;

    // Get head-to-head on this surface
    const h2hQuery = `
      SELECT 
      ...
      WHERE ((player1_id = $1 AND player2_id = $2) OR (player1_id = $2 AND player2_id = $1))
        AND COALESCE(t.surface, 'Hard') = $3
    `;
    const h2hResult = await pool.query(h2hQuery, [player1_id, player2_id, surface]);
    const p1_h2h_wins = parseInt(h2hResult.rows[0].p1_wins) || 0;
    const p2_h2h_wins = parseInt(h2hResult.rows[0].p2_wins) || 0;
    const h2h_surface_advantage = p1_h2h_wins - p2_h2h_wins;

    // Get recent form on this surface (last 10 matches)
    const formQuery = `
      SELECT player_id, COUNT(*) FILTER (WHERE winner_id = player_id) as wins
      FROM (
        SELECT m.player1_id as player_id, m.winner_id, m.match_date
        FROM matches m
        LEFT JOIN tournaments t ON m.tournament_id = t.id
        WHERE m.player1_id IN ($1, $2)
          AND COALESCE(t.surface, 'Hard') = $3
        UNION ALL
        SELECT m.player2_id as player_id, m.winner_id, m.match_date
        FROM matches m
        LEFT JOIN tournaments t ON m.tournament_id = t.id
        WHERE m.player2_id IN ($1, $2)
          AND COALESCE(t.surface, 'Hard') = $3
      ) recent
      WHERE match_date >= NOW() - INTERVAL '6 months'
      GROUP BY player_id
    `;
    const formResult = await pool.query(formQuery, [player1_id, player2_id, surface]);
    
    const form = {};
    formResult.rows.forEach(r => {
      form[r.player_id] = parseInt(r.wins) || 0;
    });

    const p1_form = form[player1_id] || 0;
    const p2_form = form[player2_id] || 0;
    const surface_form_diff = p1_form - p2_form;
...
/**
 * Helper function to make prediction using logistic regression
 */
function predictMatch(features, model) {
  // Extract feature values in correct order
  const featureValues = model.features.map(fname => features[fname]);
  
  // Standardize features
  const standardized = featureValues.map((val, i) => {
    const fname = model.features[i];
    return (val - model.scaler_mean[fname]) / model.scaler_scale[fname];
  });
  ...
  standardized.forEach((val, i) => {
    const fname = model.features[i];
    logit += val * model.coefficients[fname];
  });

  // Convert to probability using sigmoid function
  const player1_win_prob = 1 / (1 + Math.exp(-logit));
  const player2_win_prob = 1 - player1_win_prob;

  // Determine predicted winner and confidence
  const predicted_winner = player1_win_prob > 0.5 ? 1 : 2;
  const confidence = Math.abs(player1_win_prob - 0.5) * 2; // Scale 0-1

  return {
    player1_win_prob: Math.round(player1_win_prob * 1000) / 1000,
    player2_win_prob: Math.round(player2_win_prob * 1000) / 1000,
    predicted_winner,
