...
function parseAbbreviatedName(name) {
  ...
  // Check if last part ends with a dot (initial)
  ...
  // Already in First Last format, try to extract
  const lastWord = parts[parts.length - 1];
  const firstName = parts[0];
  if (lastWord.length <= 3 && firstName.length > 3) {
    // Likely "First Last" where Last is abbreviated
    return { lastName: lastWord, firstInitial: firstName[0] };
  }
...
async function importWTAMatches2025() {
  const pool = new Pool({
  ...
  try {
    ...
    for (const csvName of playerNames) {
      ...
      if (matches.length === 1) {
        ...
        // For active players in 2025, use ranking data from CSV to help
    ...
    // Clear existing 2025 matches to avoid duplicates on reimport
    console.log('Clearing existing 2025 matches...');
    const deleteResult = await client.query('DELETE FROM wta_matches WHERE EXTRACT(YEAR FROM match_date) = 2025');
...
importWTAMatches2025().catch(err => {
