...
async function debug() {
  ...
  const current = await pool.query(`
    ...
    FROM players p
    JOIN atp_player_ratings r ON p.id = r.player_id
    WHERE p.name = 'Tommy Paul'
    AND r.surface IS NULL
    AND r.rating_type = 'elo'
    AND r.id = (SELECT MAX(id) FROM atp_player_ratings WHERE player_id = p.id AND surface IS NULL AND rating_type = 'elo')
  `);
