// Calculate WTA TrueSkill Ratings: Overall (tournament-weighted)
...
async function calculateWTATrueSkillRatings() {
  console.log('Calculating WTA TrueSkill ratings (Overall only)...\n');

  // Clear existing TrueSkill ratings
  await pool.query("DELETE FROM wta_ratings WHERE rating_type = 'trueskill'");
  ...
  console.log('\n✓ Successfully calculated WTA TrueSkill ratings for all matches\n');
  ...
  console.log('WTA TrueSkill Rating Summary:');
  ...
  console.log('✓ WTA TrueSkill calculation complete!\n');
}

async function main() {
  try {
    await calculateWTATrueSkillRatings();
  } catch (error) {
    console.error('Error calculating WTA TrueSkill:', error);
  } finally {
    await pool.end();
  }
}
