// Calculate WTA Glicko2 Ratings: Overall (tournament-weighted)
...
async function calculateWTAGlicko2Ratings() {
  console.log('Calculating WTA Glicko2 ratings (Overall only)...\n');

  // Clear existing Glicko2 ratings
  await pool.query("DELETE FROM wta_ratings WHERE rating_type = 'glicko2'");
  ...
  const elapsed = Math.floor((Date.now() - startTime) / 1000);
  console.log(`\n✓ Successfully calculated WTA Glicko2 ratings for all matches\n`);
  ...
  console.log('WTA Glicko2 Rating Summary:');
  ...
  console.log('✓ WTA Glicko2 calculation complete!\n');
}
...
async function main() {
  try {
    await calculateWTAGlicko2Ratings();
    await pool.end();
  } catch (error) {
    console.error('Error calculating WTA Glicko2 ratings:', error);
    await pool.end();
    process.exit(1);
  }
}
